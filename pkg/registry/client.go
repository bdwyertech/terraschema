...
import (
	...
	"github.com/HewlettPackard/terraschema/pkg/registry/response"
...
func NewClient(client *http.Client) *Client {
	...
	retryableClient.HTTPClient.Timeout = requestTimeout
...
func (c *Client) ModuleVersions(ctx context.Context, module *regsrc.Module) (*response.ModuleVersions, error) {
	...
	log.Debugf("fetching module versions from %q", service)
	...
	for _, mod := range versions.Modules {
		for _, v := range mod.Versions {
			log.Debugf("found available version %q for %s", v.Version, mod.Source)
		}
	}
...
func (c *Client) addRequestCreds(host svchost.Hostname, req *http.Request) {
	...
	if err != nil {
		log.Warnf("Failed to get credentials for %s: %s (ignoring)", host, err)
		return
	}
...
func (c *Client) ModuleLocation(ctx context.Context, module *regsrc.Module, version string) (string, error) {
	...
	log.Debugf("looking up module location from %q", download)
